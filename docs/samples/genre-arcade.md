...
```ruby
  ...
  class Game
    # attr_gtk is a helper class macro that adds `args` attribuets to the class so you don't have to pass `args` around to every method
    attr_gtk
  ...
  def reset args
    # set the game to nil if GTK.reset is invoked so that it gets re-initialized on the next tick
    $game = nil
  end
