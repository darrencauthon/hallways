...
Given two points (each with `x` and `y` properties), this function returns a `Hash` representing a line with `x`, `y`, `x2`, `y2` properties. The line starts at point `p1` and ends at point `p2`.
...
Given a `Hash` with `x`, `y`, `x2`, `y2` properties, this function returns a `Hash` representing the midpoint of the line with `x` and `y` properties.
